...
class _Step3AttributesState extends State<Step3Attributes> {
  ...
  Future<void> _fetchAttributes() async {
    ...
    try {
      ...
      for (final attr in attributes) {
        ...
        // Create text controllers for TEXT and NUMBER types
        if (attr.type == 'TEXT' || attr.type == 'NUMBER') {
          final value = initialValues[attr.id]?.toString() ?? '';
          controllers[attr.id] = TextEditingController(text: value);
        }
      }
  ...
  Widget _buildTextInput(
    ...
    final controller = _textControllers[attribute.id];
    if (controller == null) {
    ...
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (newValue) {
          _handleAttributeChange(attribute.id, newValue);
        },
