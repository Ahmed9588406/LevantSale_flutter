...
class SessionService {
  ...
  static const String _keyIsGoogleLogin = 'is_google_login';
  ...
  static Future<bool> hasValidSession() async {
    ...
    final isGoogleLogin = prefs.getBool(_keyIsGoogleLogin) ?? false;

    if (!isLoggedIn || token == null || token.isEmpty) {
      return false;
    }

    // If Google login, check if user is still signed in with Google
    if (isGoogleLogin) {
      final isSignedIn = await GoogleAuthService.isSignedIn();
      return isSignedIn;
    }

    // For email/password login, check if credentials are saved
  ...
  static Future<Map<String, dynamic>> tryAutoLogin() async {
    try {
      ...
      if (isGoogleLogin) {
        return await _tryGoogleAutoLogin();
      }

      // Otherwise try email/password auto-login
  ...
  /// Try auto-login with Google
  static Future<Map<String, dynamic>> _tryGoogleAutoLogin() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString(_keyToken);
      ...
      if (!isSignedIn) {
        // Google user is not signed in, clear session
        await clearSession();
        return {'success': false, 'message': 'Google user not signed in'};
      }

      // User is still signed in with Google and we have a token
      ...
      return {'success': false, 'message': 'Google auto-login error: $e'};
    }
  }
  ...
  static Future<void> clearSession() async {
    ...
    await prefs.remove(_keyIsGoogleLogin);
