...
class MessagesListScreen extends StatefulWidget {
  ...
  const MessagesListScreen({super.key, this.initialSellerId, this.adData});
...
class _MessagesListScreenState extends State<MessagesListScreen> {
  ...
  // Static reference to refresh conversations from other screens
  static _MessagesListScreenState? _instance;
  ...
  // Static method to refresh conversations from other screens
  static void refreshConversations() {
    _instance?._loadConversations();
  }
  ...
  Future<void> _connectWebSocket() async {
    ...
    _socket = ChatSocket(
      ...
      onStatus: (data) {
        ...
        // When messages are marked as seen, update unread count
        final conversationId =
            data['conversationId']?.toString() ?? data['userId']?.toString();
        ...
        // Refresh conversations to get latest status after a short delay
  ...
  void _openChat(ChatConversation conversation) async {
    ...
    // Mark messages as seen using HTTP API (like web version)
    await ChatService.markMessagesSeen(conversation.id);
    ...
    // Refresh conversations when returning to ensure proper sync
    if (mounted) {
      await _loadConversations();

      // Also ensure the specific conversation is marked as read
      setState(() {
        final idx = conversations.indexWhere((c) => c.id == conversation.id);
        if (idx >= 0) {
          conversations[idx] = conversations[idx].copyWith(unreadCount: 0);
