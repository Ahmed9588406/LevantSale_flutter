...
class ChatService {
  ...
  /// Mark messages as seen via HTTP API (like web version)
  static Future<bool> markMessagesSeen(String userId) async {
    final url = Uri.parse('$baseUrl/api/v1/chat/messages/$userId/seen');
    final h = await _headers();
    final res = await http.put(url, headers: h);
    return res.statusCode >= 200 && res.statusCode < 300;
  }
